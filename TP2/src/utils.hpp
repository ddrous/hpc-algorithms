...
void CheckEqualCore(const bool v1, const bool v2, const char* file, const int line){
    if(v1 != v2){
        std::cout << "Error!" << std::endl;
        std::cout << "File: " << file << std::endl;
        std::cout << "Line: " << line << std::endl;
        std::cout << "Should be: " << (v1?"TRUE":"FALSE") << std::endl;
        std::cout << "Is: " << (v2?"TRUE":"FALSE") << std::endl;
        exit(1);
    }
}
